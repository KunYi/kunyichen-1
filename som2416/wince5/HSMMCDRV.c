...
volatile S3C2450_HSMMC_REG *g_vpHSMMCReg;	
volatile S3C2450_SYSCON_REG *g_vpSYSCONReg;	
volatile S3C2450_IOPORT_REG *g_vpIOPortReg;
...
//    HCLK = S3C2450_HCLK;
    ...
    {
        case '0':
            g_vpHSMMCReg = (volatile S3C2450_HSMMC_REG *)OALPAtoVA(S3C2450_BASE_REG_PA_HSMMC0, FALSE);
            ...
            break;
        case '1':
            g_vpHSMMCReg = (volatile S3C2450_HSMMC_REG *)OALPAtoVA(S3C2450_BASE_REG_PA_HSMMC1, FALSE);
            if (g_vpHSMMCReg == NULL)
        	{
        		RETAILMSG (1,(TEXT("[HSMMCLIB:ERR] HSMMCInit() HSMMC registers not mapped\r\n")));
        		return FALSE;
    ...
    g_vpSYSCONReg = (volatile S3C2450_SYSCON_REG *)OALPAtoVA(S3C2450_BASE_REG_PA_SYSCON, FALSE);
    ...
    g_vpIOPortReg = (volatile S3C2450_IOPORT_REG *)OALPAtoVA(S3C2450_BASE_REG_PA_IOPORT, FALSE);
    ...
    switch(ChSelect)
    {
        case '0':
            InitClkPwrCh0();
            InitGPIOCh0();
            InitHSMMCRegisterCh0();
	    RETAILMSG(1,(TEXT("### debug 0x%x # 1\r\n",g_vpHSMMCReg->CONTROL2)));
//		DPNOK(g_vpHSMMCReg->CONTROL2);
            break;
        case '1':
            InitClkPwrCh1();
            InitGPIOCh1();
            InitHSMMCRegisterCh1();
            break;
        default :
            break;
    }

  //[david.modify] 2008-08-14 10:47
//    g_vpHSMMCReg->SWRST = 0x3;
    g_vpHSMMCReg->SWRST = 0x0;

    g_vpHSMMCReg->NORINTSIGEN = 0x0;

	ClockOnOff(0);

	SetClock(SD_EPLL, 0xff);
...
//	if (!SetDataTransferWidth(HSMMC_DATA_BUSWIDTH)) // by KYS, 2416 does not support 8 bit data bus
	if (!SetDataTransferWidth(SD_DATA_BUSWIDTH))
	{
...
void InitGPIOCh0() 
{
	...
	g_vpIOPortReg->GPECON &= ~(0xFFF<<10);
	g_vpIOPortReg->GPECON |=  (0xAAA<<10);  // SD0_DAT[3:0], SD0_CMD, SD0_CLK
	g_vpIOPortReg->GPEUDP &= ~(0xFFF<<10);  // pull-up/down disabled

    // 08.04.04 by KYS
	// Setting for card detect pin of HSMMC ch0 on SMDK2450.
	g_vpIOPortReg->GPFCON = ( g_vpIOPortReg->GPFCON & ~(0x3<<2) ) | (0x2<<2);	// SD_CD0 by EINT1(GPF1)
	g_vpIOPortReg->GPFUDP = ( g_vpIOPortReg->GPFUDP & ~(0x3<<2) ) | (0x0<<2);  // pull-up/down disabled

	g_vpIOPortReg->EXTINT0 = ( g_vpIOPortReg->EXTINT0 & ~(0xF<<4) ) | (0x6<<4); // 4b'0110 : Filter Enable, Both edge triggered
}
...
void InitGPIOCh1() 
{
    ...
    // hsjang 0804028
    g_vpIOPortReg->GPLCON &= ~(0xF<<16);
    g_vpIOPortReg->GPLCON |=  (0xA<<16);	// SD1_CLK, SD0_CMD
    g_vpIOPortReg->GPLUDP &= ~(0xF<<16);	// pull-up/down disable
    g_vpIOPortReg->GPLCON &= ~(0xFF);
    g_vpIOPortReg->GPLCON |=  (0xAA);     	// SD1_DAT[3:0]
    g_vpIOPortReg->GPLUDP &= ~(0xFF);		// pull-up/down disable

    // 08.04.04 by KYS
	// Setting for card detect pin of HSMMC ch1 on SMDK2450.
	g_vpIOPortReg->GPFCON = ( g_vpIOPortReg->GPFCON & ~(0x3<<2) ) | (0x2<<2);	// SD_CD1 by EINT1(GPF1)
	g_vpIOPortReg->GPFUDP = ( g_vpIOPortReg->GPFUDP & ~(0x3<<2) ) | (0x0<<2);  // pull-up/down disabled

	g_vpIOPortReg->EXTINT0 = ( g_vpIOPortReg->EXTINT0 & ~(0xF<<4) ) | (0x6<<4); // 4b'0110 : Filter Enable, Both edge triggered
}

void InitHSMMCRegisterCh0()
{
...
void InitClkPwrCh0()
{
    ...
    g_vpSYSCONReg->CLKSRC  &= ~(0x1<<16);	// HSMMC0 clock: 0 = EPLL, 1 = USB 48MHz
    g_vpSYSCONReg->HCLKCON |= (0x1<<15);	// Enable HCLK into the HSMMC0
    g_vpSYSCONReg->SCLKCON &= ~(0x1<<13);	// Disable HSMMC_EXT clock for HSMMC 0,1 (EXTCLK)
    g_vpSYSCONReg->SCLKCON |= (0x1<<6);	// Enable HSMMC_0 clock for (EPLL/USB48M)
}
...
void InitClkPwrCh1()
{
    ...
    g_vpSYSCONReg->CLKSRC  &= ~(0x1<<17);	// HSMMC1 clock: 0 = EPLL, 1 = USB 48MHz
    g_vpSYSCONReg->HCLKCON |= (0x1<<16);	// Enable HCLK into the HSMMC1
    g_vpSYSCONReg->SCLKCON &= ~(0x1<<13);	// Disable HSMMC_EXT clock for HSMMC 0,1 (EXTCLK)
    g_vpSYSCONReg->SCLKCON |= (0x1<<12);	// Enable HSMMC_1 clock for (EPLL/USB48M)
}
