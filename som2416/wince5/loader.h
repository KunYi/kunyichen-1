...
//#define EBOOT_BLOCK_SIZE		(5)		//EBOOT 512KB以下，占2个BLOCK
//EBOOT 512KB以下，占2个BLOCK ,将原来省出来的(5-3)=2个BLOCK用于存LOGO
...
// 1. toc用了1个BLOCK，另外用一个BLOCK备份.
// 2. config.bin 将用 1个BLOCK， 1个BLOCK用来备份
// 3. LOGO区准备用2个BLOCK,2个BLOCK用来备份  (320x240, 480x272的分辩率可以满足)
// 如此一来总共将比之前多出 1+1+2+2=6个BLOCK空间用来存参数，这样大约有
// 6*256k=1.5MB
//=======================================
...
typedef struct
{
	...
	UINT32 u32LCDType;	// LCD型号
	UINT32 u32BootDelay;	//等待多久自动进入OS
	UINT32 u32TrueDevId;	// 是否使用真实的ID
	UINT8 u8DevId[16];		//如果使用手动ID，可以填入
	UINT32 u32DefLang;		// GPS UI缺省语言
	UINT8 u8MapCode[32];	//专用于PPC地图
	UINT8 u8DevCode[32];		
}stFlashCfg;
...
// 因为OS的BLOCK需要位于第10BLOCK上，所以BOOTLOADER不能越过这个
#if 0	
...
// 将LOGO BLOCK区设在EBOOT_BLOCK_RESERVED保留区上
...
#if 1
//[david.modify] 2008-09-27 10:42
// 将LOGO BLOCK区设在EBOOT_BLOCK_RESERVED保留区上, 用2个BLOCK
