...
#define RT5627_AUX_OUT_VOL					0X06			//AUXOUT VOLUME
...
#define JACK_DET_SEL_MASK			(0x3<<15)
#define JACK_DET_SEL_OFF			(0x0<<15)		//Jack Detect Select none
#define JACK_DET_SEL_GPIO			(0x1<<15)		//Jack Detect Select GPIO
#define JACK_DET_SEL_JD1			(0x2<<15)		//Jack Detect Select JD1,LineIn Left disable
#define JACK_DET_SEL_JD2			(0x3<<15)		//Jack Detect Select JD2,LineIn Right Disable
...
class RT5627:public RT_CodecComm
{
public:

		RT5627(HardwareContext *HwCxt);
		~RT5627();
		void RT_Codec_Init();
		void RT_Codec_DeInit();
		void RT_PlayBack_Start(USHORT Channel_On,USHORT Channel_Off);
		void RT_PlayBack_End(USHORT Channel_On,USHORT Channel_Off);
		void RT_Record_Start(USHORT Channel_On,USHORT Channel_Off);
		void RT_Record_End(USHORT Channel_On,USHORT Channel_Off);
		void RT_Suspend(USHORT Channel_On,USHORT Channel_Off);
		void RT_Resume(USHORT Channel_On,USHORT Channel_Off);		
		void ChangeCodecPowerStatus(BYTE power_state);
		DWORD RT_ProcessAudioMessage(UINT uMsg,DWORD dwParam1,DWORD dwParam2);
